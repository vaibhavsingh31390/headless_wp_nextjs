...
function Header({ menuData }: { menuData: MainMenu }) {
  ...
  return (
    <header className="w-full z-[9999] sticky top-0 left-0 bg-slate-800 backdrop-blur-md text-white">
      <Container className="flex justify-between items-center h-[64px]">
              ...
              menu_items.map((item) => (
                <li key={item.id} className="h-full relative group">
                  <Link
                    ...
                    className="text-transition-1 flex items-center h-full px-7"
                  >
                    {item.label}
                  </Link>
                  {item.subMenuItems.length > 0 && (
                    <div className="group-hover:block hidden bg-slate-800/50 backdrop-blur-md text-right absolute right-0 top-full">
                      <ul>
                        {item.subMenuItems.map((subMenu) => (
                          ...
                          >
                            <Link
                              ...
                              className="text-transition-1 whitespace-nowrap flex items-center h-full px-7 py-3"
                            >
                              {subMenu.label}
                            </Link>
                          </li>
